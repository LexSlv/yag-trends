...
func (repo *Repo) GetGamesTrends(offset, limit string) ([]models.Game, error) {
	selectStmt := `SELECT id, name, img, players, trend, developer, created_at, url FROM games WHERE created_at BETWEEN CURRENT_DATE - INTERVAL '1 days' AND CURRENT_DATE AND trend > 0 GROUP BY id, name, img, players, trend, developer, created_at, url ORDER BY trend DESC LIMIT $1 OFFSET $2`
	log.Info().Msg(offset)
